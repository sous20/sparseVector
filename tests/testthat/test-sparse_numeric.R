...
test_that("mean works for sparse vectors", {
  x <- as(c(1, 0, 0, 4), "sparse_numeric")
  expect_equal(mean(x), 1.25)
  ...
  # Test no zeros
  z <- as(c(2, 4, 6), "sparse_numeric")
  expect_equal(mean(z), 4)
})

test_that("norm works for sparse vectors", {
...
test_that("standardize works for sparse vectors", {
  x <- as(c(1, 2, 3, 4, 5), "sparse_numeric")
  result <- standardize(x)

  # Check that result has mean 0 and sd 1
  expect_equal(mean(result), 0, tolerance = 1e-10)

  # Test error for length 1
  y <- as(c(5), "sparse_numeric")
  expect_error(standardize(y))
})
